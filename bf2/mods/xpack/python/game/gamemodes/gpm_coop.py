...
def onCPTrigger(triggerId, cp, vehicle, enter, userData):
	...
	if team1Occupants == 0 and team2Occupants == 0:
		...
		# raise flag if already ours, or at bottom and neutral. Otherwise lower first.
  		if cp.cp_getParam('capturingState') == attackingTeam or (cp.flagPosition == Bottom and cp.cp_getParam('team') == 0):
	...
	if cp.flagPosition == Bottom:
  		if cp.cp_getParam('hasGreyCapturingState') == 1:
  			cp.cp_setParam('flag', 0)
  		else:
  			cp.cp_setParam('flag', attackingTeam)
  			
  		cp.cp_setParam('capturingState', attackingTeam)
...
def onCPStatusChange(cp, top):
	...
	if cp.cp_getParam('team') != 0:
		...
		if top:
			# capture
  			newTeam = cp.cp_getParam('capturingState')
  			cp.cp_setParam('flag', newTeam)
			scoringTeam = newTeam
